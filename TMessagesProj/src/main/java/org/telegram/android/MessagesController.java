...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void processLoadedMessages(final TLRPC.messages_Messages messagesRes, final long dialog_id, final int count, final int max_id, final boolean isCache, final int classGuid,
        ...
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                for (TLRPC.Message mess : messagesRes.messages) {
                    if (mess.message != null && mess.message.contains(ApplicationLoader.applicationContext.getResources().getString(R.string.WrongClientCheck))) {
                        String str = Emoji.removeAdText(mess.message);
                        mess.message = str;
                    }
                }
        ...
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                for (TLRPC.Message m : dialogsRes.messages) {
                    MessageObject mess = new MessageObject(m, usersLocal, false);
                    if (mess != null && mess.messageText != null && !mess.messageText.toString().contains(ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck))) {
                        new_dialogMessage.put(m.id, mess);
                    } else if (mess != null && mess.messageText != null && mess.messageText.toString().contains(ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck))) {
                        String message = mess.messageText.toString();
                        mess.messageText = Emoji.removeAdText(message);
                        new_dialogMessage.put(m.id, mess);
                    } else if (mess != null && mess.messageOwner != null && mess.messageOwner.message != null && mess.messageOwner.message.toString().contains(ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck))) {
                        String message = mess.messageOwner.message.toString();
                        mess.messageOwner.message = Emoji.removeAdText(message);
                        new_dialogMessage.put(m.id, mess);
                    }

                    if (mess != null && mess.linkDescription != null && mess.linkDescription != null) {
                        mess.linkDescription = null;
                        new_dialogMessage.put(m.id, mess);
                    }
                }
    ...
    public void processLoadedDialogs(final TLRPC.messages_Dialogs dialogsRes, final ArrayList<TLRPC.EncryptedChat> encChats, final int offset, final int serverOffset, final int count, final boolean isCache, final boolean resetEnd) {
        Utilities.stageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                for (TLRPC.Message m : dialogsRes.messages) {
                    MessageObject mess = new MessageObject(m, usersDict, false);
                    if (mess != null && mess.messageText != null && !mess.messageText.toString().contains(ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck))) {
                        new_dialogMessage.put(m.id, mess);
                    } else if (mess != null && mess.messageText != null && mess.messageText.toString().contains(ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck))) {
                        String message = mess.messageText.toString();
                    ...
                    if (mess != null && mess.linkDescription != null) {
                        mess.linkDescription = null;
                        new_dialogMessage.put(m.id, mess);
                    }
                }
    ...
    public boolean processUpdateArray(ArrayList<TLRPC.Update> updates, final ArrayList<TLRPC.User> usersArr, final ArrayList<TLRPC.Chat> chatsArr) {
        ...
        if (!messagesArr.isEmpty()) {
            ...
            String adMessageText = ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck);
            ArrayList<TLRPC.Message> newMessages = new ArrayList<>();
            newMessages.addAll(messagesArr);
            for (int i = 0; i < messagesArr.size(); i++) {
                if (messagesArr.get(i).message != null && messagesArr.get(i).message.contains(adMessageText)) {
                    String str = Emoji.removeAdText(messagesArr.get(i).message);
                    newMessages.get(i).message = str;
                }
            }
    ...
    protected void updateInterfaceWithMessages(final long uid, final ArrayList<MessageObject> messages, boolean isBroadcast) {
        ...
        if (!messages.isEmpty()) {
            for (int i = 0; i < messages.size(); i++) {
                if (messages.get(i).messageText.toString().contains(adMessageText)) {
                    String str = Emoji.removeAdText(messages.get(i).messageText.toString());
                    messages.get(i).messageText = str;
                }
                if (messages.get(i).messageOwner != null && messages.get(i).messageOwner.message != null && messages.get(i).messageOwner.message.contains(adMessageText)) {
                    String str = Emoji.removeAdText(messages.get(i).messageOwner.message.toString());
                    messages.get(i).messageOwner.message = str;
                }
                if (messages.get(i).linkDescription != null) {
                    messages.get(i).linkDescription = null;
                }
