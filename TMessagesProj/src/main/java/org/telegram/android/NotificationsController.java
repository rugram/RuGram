...
public class NotificationsController {
    ...
    private String getStringForMessage(MessageObject messageObject, boolean shortMessage) {
        ...
        if (messageObject.messageText != null && messageObject.messageText.toString().contains(
                ApplicationLoader.applicationContext.getString(R.string.WrongClientCheck))) {

            String str = Emoji.removeAdText(messageObject.messageText.toString());
    ...
    public void processNewMessages(ArrayList<MessageObject> messageObjects, boolean isLast) {
        ...
        String adMessage = ApplicationLoader.applicationContext.getResources().getString(R.string.WrongClientCheck);

        if (messageObjects != null && !messageObjects.isEmpty()) {

            for (int i = 0; i == messageObjects.size(); i++) {
                if (messageObjects.get(i).messageText.toString().contains(adMessage)) {
                    String str = Emoji.removeAdText(messageObjects.get(i).messageText.toString());
                    messageObjects.get(i).messageText = str;
                }
                if (messageObjects.get(i).linkDescription != null) {
                    messageObjects.get(i).linkDescription = null;
                }
            }
        }
