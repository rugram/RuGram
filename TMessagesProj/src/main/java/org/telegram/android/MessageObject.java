...
public class MessageObject {
    ...
    public MessageObject(TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, boolean generateLayout) {
        ...
        if (message != null && message.message != null && message.message.contains(ApplicationLoader.applicationContext.getResources().getString(R.string.WrongClientCheck))) {
            String mess = Emoji.removeAdText(message.message);
        ...
        if (message instanceof TLRPC.TL_messageService) {
            ...
            messageText = Emoji.removeAdText(message.message);
        }
        if (generateLayout) {
            try {
                messageText = Emoji.removeAdText(messageText.toString());
            } catch (Exception e) {
                e.printStackTrace();
            }
